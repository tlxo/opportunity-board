...
- Links that open in a new tab are marked as such, both visually and for
  assistive tech (WCAG 3.2.5): `NewTabLink` bundles `target="_blank"`,
...
- **Shared primitives** (`src/components/ui/`): `FocusableLink` and `Surface`
  ...
  the CSS. `NewTabLink` does the same for the new-tab indicator described
  above, so it can be styled with `styled(NewTabLink)` like any other link.
